...
class Hasta:
    def __init__(self, pid, tedavi_suresi, aciliyet):
        ...
        self.tahlil_ihtimali = 5 if self.alan == "YEŞİL" else random.randint(15, 30)
...
class HastaneSim:
    def __init__(self, root):
        self.root = root
        self.root.title("AKILLI HASTANE SİMÜLASYONU (Affinity & Asimetrik İşleme)")
    ...
    def gui_kurulum(self):
        ...
        tk.Label(ayarlar_frame, text="HİBRİT AKILLI TRİYAJ", bg="#2c3e50", fg="#f1c40f", font=("Arial", 9, "bold")).pack(side=tk.LEFT, padx=5)

        tk.Label(ayarlar_frame, text="Hız:", bg="#2c3e50", fg="white").pack(side=tk.LEFT, padx=(10,0))
        ...
        self.baslat_btn = tk.Button(action_frame, text="SİSTEMİ BAŞLAT ▶", command=self.simulasyon_toggle, bg="#2980b9", fg="white", font=("Arial", 10, "bold"))
        ...
        log_frame = tk.LabelFrame(self.sol_cerceve, text="Sistem Günlüğü", height=80, bg="#f7f9f9")
        ...
        aciklamalar = [("KIRMIZI ALAN (Acil)", "#e74c3c"), ("SARI ALAN (Orta)", "#f1c40f"), ("YEŞİL ALAN (Hafif)", "#2ecc71"), ("KAYIT/SİGORTA", "#8bc34a"), ("İLAÇ/SERUM", "#ffcc80")]
    ...
    def hasta_ekle(self):
        ...
        if "Merkezi" in self.arch_var.get():
            if self.merkezi_stok_kullanim + h.toplam_kaynak_ihtiyaci > self.TOPLAM_HASTANE_STOK:
                self.log(f"KAPASİTE DOLU: Stok yetersiz!", "error")
                return
        else:
            ...
            for i in range(self.doktor_sayisi):
                ...
                if self.bolgesel_stok_kullanim[node] + h.toplam_kaynak_ihtiyaci <= self.bolgesel_stok_boyutu:
                    ...
                    break
            if not atandi:
                self.log(f"BÖLGESEL DOLULUK: Tüm depolar dolu!", "error")
                return
    ...
    def dongu_calistir(self):
        ...
        for i in range(len(self.laboratuvar) - 1, -1, -1):
            ...
            if h.tahlil_suresi <= 0:
                ...
                self.bekleme_salonu.sort(key=lambda x: x.aciliyet)
                self.log(f"Hasta {h.pid}: Tahlil bitti. Eski doktorunu ({h.onceki_doktor_id}) arıyor.", "success")
        ...
        for i in range(self.doktor_sayisi):
            ...
            if h and not h.kaynak_cakismasi:
                if random.randint(1, 100) <= h.tahlil_ihtimali and h.kalan_sure > 5:
                    ...
                    self.odalar[i] = None
                    self.log(f"Oda-{i}: Hasta {h.pid} tahlile yollandı.", "io")
                    continue
                ...
                if h.kalan_sure <= 0:
                    ...
                    h.muayene_suresi_kullanilan = 0
                    self.bekleme_salonu.append(h)
                    self.bekleme_salonu.sort(key=lambda x: x.aciliyet) # Tekrar sıraya sok
                    self.odalar[i] = None
                    self.log(f"Oda-{i}: Hasta {h.pid} sıraya geri yollandı (Süre doldu).", "warn")
            ...
            if self.odalar[i] is None and self.bekleme_salonu:
                ...
                for hasta in self.bekleme_salonu:
                    if hasta.onceki_doktor_id == i:
                        secilen_hasta = hasta
                        self.log(f"Doktor-{i}, eski hastası {hasta.pid}'yi tanıdı ve geri aldı.", "affinity")
                        break
                
                if not secilen_hasta:
                    if i == 0: 
                        adaylar = [p for p in self.bekleme_salonu if p.alan == "YEŞİL"]
                        if adaylar:
    ...
    def kaos_tetikle(self):
        ...
        if "Çakışması" in senaryo and self.bekleme_salonu and self.doktor_sayisi > 1:
            ...
            self.kaynak_hesapla()
            self.canvas_guncelle()
            self.log("ZİNCİRLEME KAZA: Çok sayıda Acil (Kırmızı) hasta girişi!", "warn")
    ...
    def cizim_sabitleri(self):
        ...
        self.canvas.create_text(60, 190, text="TAHLİL / RÖNTGEN BEKLEYENLER", anchor="w", font=("Arial", 10, "bold"), fill="#9b59b6")
        ...
        for i in range(self.doktor_sayisi):
            ...
            rol = "GENEL"
            rol_renk = "black"
            if i == 0: rol = "YEŞİL ALAN"; rol_renk="#2ecc71"
    ...
    def hasta_ciz(self, x, y, h, aktif=False):
        ...
        if h.durum == "TAHLİLDE": cerceve = "#9b59b6"; kalinlik=3
        else: kalinlik = 2
        ...
        durum_metni = "KİLİTLENDİ" if h.kaynak_cakismasi else (f"Tahlil ({h.tahlil_suresi}dk)" if h.durum == "TAHLİLDE" else f"Kalan: {h.kalan_sure}dk")
        self.canvas.create_text(x+w/2, y+60, text=durum_metni, font=("Arial", 9, "bold" if h.durum=="TAHLİLDE" else "normal"))
    ...
    def dosya_detay_ciz(self, hasta):
        ...
        if hasta.yanlis_teshis:
            ...
            c.create_text(w/2, curr_y+kernel_h/2, text="YANLIŞ DOSYA!", font=("Arial", 12, "bold"), fill="white")
            return
